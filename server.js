...
app.post('/login', async (req, res) => {
    ...
    try {
        ...
        if (result.rows.length > 0) { 
            ...
            res.send("Xato login yoki parol! <a href='/login'>Qaytish</a>"); 
        }
...
app.post('/add', async (req, res) => {
    ...
    try {
        await pool.query('INSERT INTO loads (driver, dest, lat, lng, status) VALUES ($1, $2, 41.2995, 69.2401, $3)', [driver, dest, 'Active']);
        res.redirect('/');
    } catch (e) { res.status(500).send(e.message); }
});
...
setInterval(async () => {
    ...
    } catch (e) { console.log("Simulyator xatosi: ", e.message); }
}, 4000);

const PORT = process.env.PORT || 10000;
server.listen(PORT, () => console.log("✅ Server yondi: PORT " + PORT));
